...
# =========================================================================
    ...
    for n_O_val in range(0, 6):  # Up to 5 oxygen atoms
        mask = np.array([meta['atom_counts'].get('O', 0) == n_O_val for meta in metadata_list])
        if np.sum(mask) >= 50:
            bias = np.mean(res_after_size[mask])
            if abs(bias * HARTREE_TO_KCAL) > 0.5:
                HETEROATOM_CORRECTIONS[f'O_{n_O_val}'] = bias
                print(f"    O={n_O_val}: bias = {bias * HARTREE_TO_KCAL:.2f} kcal/mol")

    # Apply heteroatom corrections
    hetero_corr = np.zeros(len(X))
    for i, meta in enumerate(metadata_list):
        n_O = meta['atom_counts'].get('O', 0)
        key = f'O_{n_O}'
        if key in HETEROATOM_CORRECTIONS:
            hetero_corr[i] += HETEROATOM_CORRECTIONS[key] 
    
    print("\n[Nitrogen Type Corrections]")
    N_TYPE_CORRECTIONS = {}
    
    # Get N-type counts from features
    ...
    for ntype_feat in n_type_features:
        feat_idx = feature_names.index(ntype_feat) if ntype_feat in feature_names else None
        if feat_idx is not None:
            for count_val in range(1, 4):  # 1, 2, or 3 of this N type
                mask = X[:, feat_idx] == count_val
                if np.sum(mask) >= 30:
                    bias = np.mean(res_after_size[mask])
                    if abs(bias * HARTREE_TO_KCAL) > 0.3:
                        key = f'{ntype_feat}_{count_val}'
                        N_TYPE_CORRECTIONS[key] = bias
                        print(f"    {key}: bias = {bias * HARTREE_TO_KCAL:.2f} kcal/mol (n={np.sum(mask)})")
    
    # Apply N-type corrections
    n_type_corr = np.zeros(len(X))
    for i, fd in enumerate(feature_dicts):
        for ntype_feat in n_type_features:
            count_val = int(fd.get(ntype_feat, 0))
            key = f'{ntype_feat}_{count_val}'
            if key in N_TYPE_CORRECTIONS:
                n_type_corr[i] += N_TYPE_CORRECTIONS[key]
    
    print("\n[Oxygen Type Corrections]")
    O_TYPE_CORRECTIONS = {}
    ...
    for otype_feat in o_type_features:
        feat_idx = feature_names.index(otype_feat) if otype_feat in feature_names else None
        if feat_idx is not None:
            for count_val in range(1, 4):
                mask = X[:, feat_idx] == count_val
                if np.sum(mask) >= 30:
                    bias = np.mean(res_after_size[mask])
                    if abs(bias * HARTREE_TO_KCAL) > 0.3:
                        key = f'{otype_feat}_{count_val}'
                        O_TYPE_CORRECTIONS[key] = bias
                        print(f"    {key}: bias = {bias * HARTREE_TO_KCAL:.2f} kcal/mol (n={np.sum(mask)})")
    
    # Apply O-type corrections
    o_type_corr = np.zeros(len(X))
    for i, fd in enumerate(feature_dicts):
        for otype_feat in o_type_features:
            count_val = int(fd.get(otype_feat, 0))
            key = f'{otype_feat}_{count_val}'
            if key in O_TYPE_CORRECTIONS:
                o_type_corr[i] += O_TYPE_CORRECTIONS[key]
    
    print("\n[Aromatic-Heteroatom Corrections]")
    AROM_HET_CORRECTIONS = {}
    
    # Focus on methoxy/alkoxy on aromatic - your problem case
    ...
    for arom_feat in arom_features:
        feat_idx = feature_names.index(arom_feat) if arom_feat in feature_names else None
        if feat_idx is not None:
            for count_val in range(1, 4):
                mask = X[:, feat_idx] == count_val
                if np.sum(mask) >= 20:
                    bias = np.mean(res_after_size[mask])
                    if abs(bias * HARTREE_TO_KCAL) > 0.2:
                        key = f'{arom_feat}_{count_val}'
                        AROM_HET_CORRECTIONS[key] = bias
                        print(f"    {key}: bias = {bias * HARTREE_TO_KCAL:.2f} kcal/mol (n={np.sum(mask)})")
    
    # Apply aromatic-heteroatom corrections
    arom_het_corr = np.zeros(len(X))
    for i, fd in enumerate(feature_dicts):
        for arom_feat in arom_features:
            count_val = int(fd.get(arom_feat, 0))
            key = f'{arom_feat}_{count_val}'
            if key in AROM_HET_CORRECTIONS:
                arom_het_corr[i] += AROM_HET_CORRECTIONS[key]

    print("\n[Size-Stratified Corrections]")   
    SIZE_HETERO_CORRECTIONS = {} 
    size_bins = [(1, 5), (5, 7), (7, 9)]
    
    for size_min, size_max in size_bins:
        size_mask = (n_heavy >= size_min) & (n_heavy < size_max)
        
        for n_O_val in range(0, 4):
            o_mask = np.array([meta['atom_counts'].get('O', 0) == n_O_val for meta in metadata_list])
            combined_mask = size_mask & o_mask
            
            if np.sum(combined_mask) >= 50:
                bias = np.mean(res_after_size[combined_mask])
                if abs(bias * HARTREE_TO_KCAL) > 0.3:
                    key = f'size_{size_min}_{size_max}_O_{n_O_val}'
                    SIZE_HETERO_CORRECTIONS[key] = bias
                    print(f"    {key}: bias = {bias * HARTREE_TO_KCAL:.2f} kcal/mol (n={np.sum(combined_mask)})")
    
    # Apply size-hetero corrections
    size_hetero_corr = np.zeros(len(X))
    for i, meta in enumerate(metadata_list):
        n_O_i = meta['atom_counts'].get('O', 0)
        n_heavy_i = n_heavy[i]
        ...
        if n_heavy_i < 5:
            ...
            size_bin = None  # At boundary of training data
        
        if size_bin is not None:
            key = f'size_{size_bin[0]}_{size_bin[1]}_O_{n_O_i}'
            if key in SIZE_HETERO_CORRECTIONS:
                size_hetero_corr[i] += SIZE_HETERO_CORRECTIONS[key]
    
    print("\n    Analyzing size-O correction trends...")
    
    size_centers = []
    o_corrections_by_size = []
    
    for size_val in range(3, 10):
        size_mask = n_heavy == size_val
        o_mask = np.array([meta['atom_counts'].get('O', 0) >= 1 for meta in metadata_list])
        combined = size_mask & o_mask
        
        if np.sum(combined) >= 30:
            correction = np.mean(res_after_size[combined])
            size_centers.append(size_val)
            o_corrections_by_size.append(correction)
            print(f"        Size {size_val}, O>=1: correction = {correction * HARTREE_TO_KCAL:.2f} kcal/mol")
    
    if len(size_centers) >= 3:
        O_SIZE_TREND_COEFFS = np.polyfit(size_centers, o_corrections_by_size, 1)
        print(f"\n    O-size trend: {O_SIZE_TREND_COEFFS[0]*HARTREE_TO_KCAL:.4f} kcal/mol per heavy atom")
    else:
        O_SIZE_TREND_COEFFS = [0, 0]
    ...
    # Include ALL corrections in the final formation total
    pred_formation_total = (pred_formation_ml + size_corr + fg_corr + hetero_corr + n_type_corr + o_type_corr + arom_het_corr + size_hetero_corr)
    ...
    MODEL_COMPONENTS = {
        ...
        'heteroatom_corrections': HETEROATOM_CORRECTIONS,
        'n_type_corrections': N_TYPE_CORRECTIONS,
        'o_type_corrections': O_TYPE_CORRECTIONS,
        'arom_het_corrections': AROM_HET_CORRECTIONS,
        'size_hetero_corrections': SIZE_HETERO_CORRECTIONS,
        'o_size_trend_coeffs': O_SIZE_TREND_COEFFS,
    }
